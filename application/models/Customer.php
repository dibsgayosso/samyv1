		...
		if (isset($run_webhook) && $run_webhook)
		{
			...
			if(@$customer['email'] || @$customer['phone_number']){
				...
				$customer_data_sidekick = array(
					...
					"source" => "public api php point of sale",
				);
...
full_name LIKE '".($this->config->item('customer_allow_partial_match') ? '%' : '').$this->db->escape_like_str($search)."%') and deleted=$deleted and location_id = $current_location");
