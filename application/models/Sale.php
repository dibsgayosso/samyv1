...
$this->load->model('Sale_types');
		...
		if($cart->create_work_order)
		{
			...
			'last_modified' => $sale_id ? date('Y-m-d H:i:s') : NULL,
                ...
                if ($sale_id && $before_save_sale_info && isset($before_save_sale_info->location_folio))
                {
                        $sales_data['location_folio'] = $before_save_sale_info->location_folio;
                }
                else if(!$sale_id)
                {
                        $sales_data['location_folio'] = $this->get_next_location_folio($location_id);
                }
	...
	function get_sales_for_customer_by_items($item_id, $customer_id)
	{
		$return = array();

		$this->db->select('sales.sale_time, sales_items.item_unit_price, sales_items.discount_percent')
			->from('sales')
			->join('sales_items', 'sales.sale_id = sales_items.sale_id')
	...
	*/
	...
	public function get_location_folio($sale_id)
	{
		$sale_row = $this->get_info($sale_id)->row();
		...
		if (isset($sale_row->location_folio) && $sale_row->location_folio)
		{
			return (int)$sale_row->location_folio;
		}

		$location_folio = $this->get_location_folio_from_history($sale_row->location_id, $sale_id);

		if (!$location_folio)
		{
			// Ensure we always have a visible folio even if historical counts return 0
			$location_folio = (int)$sale_row->sale_id;
		}

		$this->db->where('sale_id', $sale_id);
		$this->db->update('sales', array('location_folio' => $location_folio));

		return $location_folio;
	}

	public function get_next_location_folio($location_id)
        {
                $this->db->select('MAX(location_folio) AS max_folio', false);
                $this->db->from('sales');
                $this->db->where('location_id', $location_id);

                $result = $this->db->get()->row();

                return (int)$result->max_folio + 1;
        }

        protected function get_location_folio_from_history($location_id, $sale_id)
        {
                $this->db->from('sales');
                $this->db->where('location_id', $location_id);
                $this->db->where('sale_id <=', $sale_id);
