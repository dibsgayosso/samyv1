	...
	*/
	function get_item_search_suggestions_without_variations($search,$deleted = 0,$limit=25,$price_field = FALSE,$hide_inactive = false, $supplier_id = false)
	{
		...
		if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
		{
			if (strpos($search, '.') !== false)
			{
				...
				$this->db->select('items.item_id, item_number, items.main_image_id as image_id, items.name, categories.name as category');
				$this->db->from('items');
				$this->db->join('categories', 'categories.id = items.category_id','left');
				$this->db->like('items.item_number', $search, !$this->config->item('speed_up_search_queries') ? 'both' : 'after');
				$this->db->where('items.deleted',$deleted);
				...
				$this->db->group_by('items.item_id');
				$this->db->limit($limit);
				$by_item_number_decimal = $this->db->get();

				$temp_suggestions = array();
				foreach($by_item_number_decimal->result() as $row)
				{
					$data = array(
						'avatar' => $row->image_id && !$this->config->item('dont_show_images_in_search_suggestions') ?  cacheable_app_file_url($row->image_id) : base_url()."assets/img/item.png" ,
						'category' => $row->category,
	...
	function get_item_search_suggestions($search,$deleted=0,$price_field = 'unit_price',$limit=25,$hide_inactive = false, $supplier_id = false)
	{
		...
		if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
		{
			if (strpos($search, '.') !== false)
			{
				$this->db->select('items.item_id, items.item_number, items.size, items.name');
				if ($price_field) {
					$this->db->select('items.'. $price_field); 
				}
				$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,items.main_image_id as image_id, item_variations.id as item_variation_id, categories.name as category, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
				$this->db->from('items');
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
				$this->db->join('item_variations', 'items.item_id = item_variations.item_id','left');
	...
	*/
	...
	function merge($items,$item_to_merge)
	{
		...
		if (count($items) > 0)
		{
			...
			$this->Inventory->external_inventory_update($updateInventoryArr);
