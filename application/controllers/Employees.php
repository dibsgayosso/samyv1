...
function active_employees_template()
{
$this->check_action_permission('excel_export');
ini_set('memory_limit','1024M');
set_time_limit(0);
...
// Pull only active, non-deleted employees to build the template
$employees = $this->Employee->get_all(0, 10000, 0, 'last_name', 'asc', false);
...
foreach ($employees->result_object() as $employee)
{
...
$this->load->helper('spreadsheet');
array_to_spreadsheet($rows, 'active_employees_template.'.($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
}

function active_template_import()
{
$this->check_action_permission('add_update');
$this->load->view('employees/active_template_import');
}

function do_active_template_import()
{
$this->check_action_permission('add_update');

if (empty($_FILES['file_path']['tmp_name']))
{
echo json_encode(array('success' => false, 'message' => lang('common_full_path_to_excel_required')));
return;
}

$this->load->helper('spreadsheet');
$file_info = pathinfo($_FILES["file_path"]["name"]);
$extension = isset($file_info['extension']) ? strtolower($file_info['extension']) : 'xlsx';

$tmpFilename = tempnam(ini_get('upload_tmp_dir') ? ini_get('upload_tmp_dir') : sys_get_temp_dir(), 'empimport');
file_put_contents($tmpFilename,file_get_contents($_FILES["file_path"]["tmp_name"]));

$spreadsheet = PHPPOSSpreadsheet::getSpreadsheetClass($tmpFilename,$extension);
$total_rows = $spreadsheet && method_exists($spreadsheet,'getNumberOfRows') ? (int)$spreadsheet->getNumberOfRows() : 0;

if(!$spreadsheet || $total_rows <= 1)
{
@unlink($tmpFilename);
echo json_encode(array('success' => false, 'message' => lang('employees_active_template_import_empty')));
return;
}
...
{
$person_id_cell = trim((string)$spreadsheet->getCellByColumnAndRow(5,$row));
$is_new_person = $person_id_cell === '';
...
$max_row = $this->db->get('people')->row();
$next_person_id = $max_row && isset($max_row->person_id) ? (int)$max_row->person_id : 0;
}
...
{
$person_id = (int)$person_id_cell;
}

if (!$is_new_person && !$this->Employee->exists($person_id))
{
$failures[] = $person_id ? $person_id : lang('employees_missing_person_id');
continue;
}

$person_data = array(
'first_name'=>trim((string)$spreadsheet->getCellByColumnAndRow(1,$row)),
'last_name'=>trim((string)$spreadsheet->getCellByColumnAndRow(2,$row)),
'email'=>trim((string)$spreadsheet->getCellByColumnAndRow(3,$row)),
'phone_number'=>trim((string)$spreadsheet->getCellByColumnAndRow(4,$row)),
);
...
$employee_data = array('inactive' => 0);
$username = trim((string)$spreadsheet->getCellByColumnAndRow(0,$row));
if ($username)
{
$employee_data['username'] = $username;
}

if ($this->Employee->save_profile($person_data,$employee_data,$person_id))
{
$success_count++;
}
else
...
$message = sprintf(lang('employees_active_template_import_success'), $success_count);
if (count($failures))
{
$message .= ' '.sprintf(lang('employees_active_template_import_errors'), implode(', ', $failures));
}

echo json_encode(array('success' => count($failures) === 0, 'message' => $message));
}
