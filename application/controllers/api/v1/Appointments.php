...
class Appointments extends REST_Controller {
    ...
    function __construct()
    {
		...
		private function _appointments_result_to_array($appointments)
		{
			$this->load->helper('date');
				...
				$appointments_return['employee_phone_number'] = format_phone_number($employee->phone_number);
				...
				$appointments_return['customer_phone_number'] = format_phone_number($customer->phone_number);
    ...
    private function _create_appointments($appointments_request)
    {
			...
			if (isset($appointments_request['first_name']))
			{
				$person_data = array(
				'first_name'=>isset($appointments_request['first_name']) ? $appointments_request['first_name'] : '',
				'last_name'=>isset($appointments_request['last_name']) ? $appointments_request['last_name'] : '',
				'email'=>isset($appointments_request['email']) ? $appointments_request['email'] : '',
				'phone_number'=>isset($appointments_request['phone_number']) ? $appointments_request['phone_number'] : '',
				'address_1'=>isset($appointments_request['address_1']) ? $appointments_request['address_1'] : '',
				'address_2'=>isset($appointments_request['address_2']) ? $appointments_request['address_2'] : '',
				'city'=>isset($appointments_request['city']) ? $appointments_request['city'] : '',
				'state'=>isset($appointments_request['state']) ? $appointments_request['state'] : '',
				'zip'=>isset($appointments_request['zip']) ? $appointments_request['zip'] : '',
				'country'=>isset($appointments_request['country']) ? $appointments_request['country'] : '',
				'comments'=>isset($appointments_request['comments']) ? $appointments_request['comments'] : '',
				);
		
				$customer_data=array(
					'company_name'=>isset($appointments_request['company_name']) ? $appointments_request['company_name'] : '',
					'tier_id'=>isset($appointments_request['tier_id']) &&  $appointments_request['tier_id'] ? $appointments_request['tier_id'] : NULL,
					'account_number'=>isset($appointments_request['account_number']) ? $appointments_request['account_number'] : NULL,
					'taxable'=>(!isset($appointments_request['taxable']) || (isset($appointments_request['taxable']) && $appointments_request['taxable'])) ? 1 : 0,
					'tax_certificate'=>isset($appointments_request['tax_certificate']) ? $appointments_request['tax_certificate'] : '',
					'override_default_tax'=>isset($appointments_request['override_default_tax']) && $appointments_request['override_default_tax'] ? 1 : 0,
					'tax_class_id'=>isset($appointments_request['tax_class_id']) ? $appointments_request['tax_class_id'] : NULL,
					'balance'=>isset($appointments_request['balance']) ? $appointments_request['balance'] : 0,
					'credit_limit'=>isset($appointments_request['credit_limit']) ? $appointments_request['credit_limit'] : NULL,
					'points'=>isset($appointments_request['points']) ? $appointments_request['points'] : 0,
					'internal_notes'=>isset($appointments_request['internal_notes']) ? $appointments_request['internal_notes'] : '',
					'disable_loyalty'=>isset($appointments_request['disable_loyalty']) && $appointments_request['disable_loyalty'] ? 1 : 0,
					'location_id'=>isset($appointments_request['location_id']) && $appointments_request['location_id'] ? $appointments_request['location_id'] : NULL,
					'customer_info_popup' => isset($appointments_request['customer_info_popup']) && $appointments_request['customer_info_popup'] ? $appointments_request['customer_info_popup'] : NULL,
					'auto_email_receipt' => isset($appointments_request['auto_email_receipt']) && $appointments_request['auto_email_receipt'] ? 1 : 0,
					'auto_email_receipt' => isset($appointments_request['always_sms_receipt']) && $appointments_request['always_sms_receipt'] ? 1 : 0,
				);
			
				$existing_customer_info = $this->Customer->get_info_by_email($person_data['email']);
				$this->Customer->save_customer($person_data,$customer_data, $existing_customer_info->person_id ? $existing_customer_info->person_id: FALSE,isset($appointments_request['skip_webhook']) && $appointments_request['skip_webhook'] ? TRUE : FALSE);
			
			
				$appointments_request['customer_id'] = $existing_customer_info->person_id ? $existing_customer_info->person_id : $customer_data['person_id'];
			}
			...
			$appointments_data=array(
				...
				'end_time' => isset($appointments_request['end_time']) && $appointments_request['end_time'] ? date('Y-m-d H:i:s',strtotime($appointments_request['end_time'])) : date('Y-m-d H:i:s'),
				'person_id' => isset($appointments_request['customer_id']) && $appointments_request['customer_id'] ? $appointments_request['customer_id'] : NULL,
				'employee_id' => isset($appointments_request['employee_id']) && $appointments_request['employee_id'] ? $appointments_request['employee_id'] : 1,
				'appointments_type_id' => isset($appointments_request['appointments_type_id']) && $appointments_request['appointments_type_id'] ? $appointments_request['appointments_type_id'] : 1,
				'notes' => isset($appointments_request['notes']) && $appointments_request['notes'] ? $appointments_request['notes'] : '',
				
			);

			if($this->Appointment->save($appointments_data)){
				$this->Appointment->send_email($appointments_data);
    ...
    private function _update_appointments($appointments_id,$appointments_request)
    {
  		...
  		$appointments = $this->Appointment->get_info($appointments_id);
			...
			{
				$this->Appointment->send_email($appointments_request);
