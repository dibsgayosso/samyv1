		...
		<div class="modal-header" id="myTabHeader">
			...
			<nav>
				<ul id="myTab" class="nav nav-tabs nav-justified">
					...
					<?php if(isset($customer_id) && !is_null($customer_id)) { ?>
					<li><a href="#CustomerInfo" data-toggle="tab"><?php echo lang('common_customer_info'); ?></a></li>
					<?php } ?>
...
<script>
	...
	$(".change-price-btn").on('click', function() {
		...
		$.ajax({
			url: '<?php echo site_url("sales/edit_item/"); ?>' + line,
			type: 'POST',
			data: {
				'name': 'unit_price',
				'value': price,
