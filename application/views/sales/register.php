...
<?php if (!empty($sale_workspaces)) { ?>
<style>
.sale-workspace-bar {margin-bottom: 10px;}
.sale-workspace-cards {display: flex; flex-wrap: wrap; gap: 8px; align-items: stretch;}
.sale-workspace-card {position: relative; border: 1px solid #ddd; border-radius: 6px; background: #fafafa; min-width: 220px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);} 
.sale-workspace-card.active {border-color: #5bc0de; box-shadow: 0 0 0 2px rgba(91,192,222,0.2);} 
.sale-workspace-card .workspace-card-body {display: block; padding: 10px 12px; color: inherit; text-decoration: none;} 
.sale-workspace-card .workspace-label {font-weight: 600; font-size: 14px;} 
.sale-workspace-card .workspace-meta {margin-top: 6px; font-size: 12px; display: flex; flex-wrap: wrap; gap: 6px;} 
.sale-workspace-card .workspace-close {position: absolute; top: 6px; right: 8px; color: #888;} 
.sale-workspace-card .workspace-close:hover {color: #444;}
.sale-workspace-actions {margin-bottom: 6px;}
</style>

<div class="sale-workspace-bar hidden-print">
       <div class="sale-workspace-actions">
               <a class="btn btn-primary btn-sm" href="<?php echo site_url('sales/new_sale_workspace'); ?>">
                       <i class="ion-plus"></i> <?php echo lang('sales_new_sale_workspace'); ?>
               </a>
       </div>
       <div class="sale-workspace-cards">
       <?php foreach($sale_workspaces as $workspace) { ?>
               <div class="sale-workspace-card <?php echo $workspace['is_active'] ? 'active' : ''; ?>">
                       <a class="workspace-card-body <?php echo $workspace['is_active'] ? 'active' : ''; ?>" <?php echo $workspace['is_active'] ? 'href="#" onclick="return false;"' : 'href="'.site_url('sales/switch_sale_workspace/'.rawurlencode($workspace['id'])).'"'; ?>>
                               <div class="workspace-label"><?php echo H($workspace['label']); ?></div>
                               <div class="workspace-meta">
                                       <span class="workspace-customer"><?php echo H($workspace['customer']); ?></span>
                                       <span class="workspace-total"><?php echo $workspace['total']; ?></span>
                                       <span class="workspace-items"><?php echo to_quantity($workspace['item_count']); ?> <?php echo lang('common_items'); ?></span>
                               </div>
                       </a>
                       <?php if ($workspace['is_closable']) { ?>
                               <a class="workspace-close" href="<?php echo site_url('sales/close_sale_workspace/'.rawurlencode($workspace['id'])); ?>" title="<?php echo lang('sales_close_sale_workspace'); ?>">&times;</a>
                       <?php } ?>
               </div>
       <?php } ?>
       </div>
									...
									<tbody class="register-item-content" data-line="<?php echo $line; ?>">
										<tr class="register-item-details">
											...
											<td>
												...
												<a tabindex="-1" href="<?php echo isset($item->item_id) ? site_url('home/view_item_modal/' . $item->item_id . '/' . $line . '/' . $customer_id) . "?redirect=sales" : site_url('home/view_item_kit_modal/' . $item->item_kit_id) . "?redirect=sales"; ?>" data-toggle="modal" data-target="#myModal" class="register-item-name"><?php echo H($item->name).(property_exists($item, 'variation_name') && $item->variation_name ? '<span class="show-collpased" style="display:none">  ['.$item->variation_name.']</span>' : '') ?><?php echo $item->size ? ' (' . H($item->size) . ')' : ''; ?></a>
											</td>
											...
											<td class="text-center">
												<?php
												if ($item->product_id != lang('common_integrated_gift_card') && (!$cart->suspended || $this->Employee->has_module_action_permission('sales', 'edit_suspended_sale_data', $this->Employee->get_logged_in_employee_info()->person_id)) && $this->config->item('disable_discounts_percentage_per_line_item') != 1) {
												?>
			...
			<table id="register" class="table table-hover ">
				...
				<tbody id="cart_contents">
					...
					?>
						...
						<tr id="reg_item_top">
							<td class="text text-center text-success"><a tabindex="-1" href="<?php echo isset($item->item_id) ? site_url("home/view_item_modal/" . $item->item_id . '/'. $line . '/' . $customer_id) : site_url('home/view_item_kit_modal/' . $item->item_kit_id . "?redirect=sales"); ?>" data-toggle="modal" data-target="#myModal"><?php echo H($item->name); ?></a></td>
							<td class="text-center">
								<?php
					...
					<?php if ($mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
						...
						<?php if ($this->config->item('customers_store_accounts')) { ?>
							...
							<?php if ($this->Employee->has_module_action_permission('sales', 'receive_store_account_payment', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
                                                        ...
                                                        <li>
                                                                <?php echo anchor(
                                                                        "sales/branch_sale_search",
                                                                        '<i class="ion-search"></i> ' . lang('sales_branch_sale_search'),
                                                                        array('class' => '', 'title' => lang('sales_branch_sale_search'))
                                                                );
                                                                ?>
                                                        </li>
...
?>
	...
	<?php } ?>
        ...
        <div class="customer-form salesperson-form">

                <?php echo form_open("sales/set_sold_by_employee_id", array('id' => 'select_salesperson_form', 'autocomplete' => 'off', 'class' => 'form-inline')); ?>
                <div class="input-group contacts">
                        <span class="input-group-addon">
                                <i class="ion-person"></i>
                        </span>
                        <input type="number" id="sold_by_employee" name="sold_by_employee_id" class="add-customer-input keyboardLeft" data-title="<?php echo lang('common_sales_person'); ?>" value="<?php echo $selected_sold_by_employee_id ? H($selected_sold_by_employee_id) : ''; ?>" placeholder="<?php echo lang('sales_enter_salesperson_id'); ?>" min="1">
                </div>
                </form>
                ...
                <?php if ($this->config->item('select_sales_person_during_sale')) {  ?>
                        <div class="tier-group">
                                <a href="#" class="select-sales-person <?php $this->config->item('select_sales_person_during_sale') ? 'enable-click' : ''; ?>">
                                        <?php echo lang('common_sales_person'); ?>: <span class="selected-sales-person"><?php echo H($selected_salesperson_name); ?></span>
                                </a>
...
?>
	...
	<?php } ?>
                ...
                if ($('#sold_by_employee').length)
                {
                        const submitSoldByEmployeeId = function() {
                                $.post('<?php echo site_url("sales/set_sold_by_employee_id"); ?>', {
                                        sold_by_employee_id: $("#sold_by_employee").val()
                                }, function(response) {
                                        $("#register_container").html(response);
                                });
                        };

                        $('#sold_by_employee').on('change blur', submitSoldByEmployeeId);

                        $('#sold_by_employee').on('keyup', function(event) {
                                if (event.keyCode == 13)
                                {
                                        submitSoldByEmployeeId();
                                }
                        });
                }
